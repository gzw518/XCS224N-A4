...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=self.model_embeddings.embed_size, hidden_size=self.hidden_size, num_layers=1, bias=True, batch_first=False, dropout=self.dropout_rate, bidirectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ### TODO:
        ###     1. Compute tensor `X` of source sentences with shape (src_len, b, e) using the source model embeddings.
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_list = torch.split(Y, split_size_or_sections=1, dim=0)
        for Y_t in Y_list:
            Y_t = torch.squeeze(Y_t, dim=0)                   # Y_t shape (b,h)
            Ybar_t = torch.cat((Y_t, o_prev), dim=-1)         # o_prev shape (b,h), Ybar_t shape (b, 2xh)
